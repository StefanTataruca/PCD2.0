...
void convert_xml_to_json(const char *xml_path, const char *json_path) {
    ...
    snprintf(log_path, sizeof(log_path), "%s.log", xml_path);
    FILE *log_file = fopen(log_path, "a");
    if (log_file) {
        fprintf(log_file, "Converted XML file '%s' to JSON file '%s'\n", xml_path, json_path);
        fclose(log_file);
    }
}
    ...
    if (strcmp(role, "admin") == 0) {
        ...
        while (1) {
            ...
            if (strcmp(buffer, "upload") == 0) {
                ...
                // Create log file
                char log_filename[BUFFER_SIZE * 2]; // Am mărit dimensiunea bufferului pentru a evita trunchierea
                snprintf(log_filename, sizeof(log_filename), "%s.log", xml_path);
                FILE *log_file = fopen(log_filename, "a");
                if (log_file) {
                    fprintf(log_file, "Uploaded XML file '%s' and created JSON file '%s'\n", xml_path, json_filename);
                    fclose(log_file);
                }
